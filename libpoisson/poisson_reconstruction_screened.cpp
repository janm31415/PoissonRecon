  ...
  const poisson_reconstruction_screened_parameters& par)
{
  ...
  triangles.clear();
  vertex_colors.clear();
  int MaxDepthVal = par.depth;
  int MaxSolveDepthVal = -1;
  int KernelDepthVal = -1;
  int MinDepthVal = 0;
  int FullDepthVal = par.octree_depth;
  Real SamplesPerNodeVal = par.samples_per_node;
  Real ScaleVal = par.scale_factor;
  bool ConfidenceFlag = false;
  bool CleanFlag = false;
  bool DensityFlag = false;
  Real PointWeightVal = par.interpolation_weight;
  int AdaptiveExponentVal = 1;
  int BoundaryTypeVal = 1;
  bool CompleteFlag = false;
  bool NonManifoldFlag = false;
  bool ShowResidualFlag = false;
  int CGDepthVal = par.conjugate_gradients_depth;
  int ItersVal = par.conjugate_gradients_iterations;
  Real CSSolverAccuracyVal = 1e-3f;
